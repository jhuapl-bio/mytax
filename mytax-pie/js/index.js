...
// upload_button('uploader', load_file);
...
function parse_Children(children){
	...
	for(var key in children){
		if (key.children != null){
			var ch = parse_Children(key.children)
		}
	}
}
...
// file = "test-data/BC03.report.full.json"
var old_file = ""; var newer = "";
// load_file(file, 0)
$('#filter_taxa').click("on", function(){
	console.log(old_file)
	load_file(old_file, 1)
})
setInterval(function(){
	var x  = document.getElementById("uploader").files[0]
	console.log(x)
	var reader = new FileReader();
	reader.onload = function(e) {
    // The file's text will be printed here
    	if (newer && old_file && newer !== old_file) {
	    	newer = event.target.result

			console.log("yes")
		    console.log('refresh');
			load_file(event.target.result,1)
		}
  	};
  	reader.readAsText(x)
...
$('#select_all_taxa').on("click", function(){
	($('#taxa_select').find("input").each(function(){
		console.log(this)
...
function load_file(file, init){
   		old_file = file            
    	var data = JSON.parse(file)
    	console.log(data)
    	var accepted   = []
    	if (init ==0){
    	for (let [key, value] of Object.entries(data.metadata.fields)){
				$('#taxa_select').append('<input checked type="checkbox" value=' +value.name+'>'+value.name+' <br>')
				accepted.push(value.name)
			}
	    }
		($('#taxa_select').find("input").each(function(){
			if ($(this).is(':checked')){
				accepted.push(this.value)
			}
		}))
    	load_dataset(data, accepted)
}
...
function load_dataset(data, accepted) {
	console.log(data)
	var y = (flatten(data))
	var acc = {}
	for (let [key, value] of Object.entries(y)) {
		var split = key.split(".")
		var el = split[split.length-1]	
		if (el == "rank" && accepted.includes(value))	{
			acc[split.splice(0,split.length-1).join(".")] = 0
		}		
	}
	var good = {}
	var count = {}
	...
	for (let [key, value] of Object.entries(acc)) {
		var split = key.split(".")
		var el = split[split.length-1]	
		var trunc = split.slice(0)
		var stry2  = ""
		var stry = String(split.join("."))
		if (key != ""){
			while(!(stry2 in good) && trunc.length > 0){
				trunc.splice(-1,1)
				stry2 = trunc.join(".")
			}
			if (trunc.length ==0 && split.length > 0){
				stry2 = "children["+root_acc+"]"
				root_acc +=1
				good[stry] = stry2
			}
			else{
				...
				good[stry] = good[stry2]+".children["+count[stry2]+"]"
				count[stry2]+=1
			}
		}
		else{
	...
	for (let [key, value] of Object.entries(y)) {
		var split = key.split(".")
		var trunc = (split.splice(0, split.length-1))
		var stry = String(trunc.join("."))
		if (!(stry in good)){
			delete y[key]
		}
		else{
			new_good[String(good[stry]+"."+split[split.length-1])] = value
		}
	}
	y = unflatten(new_good)
	var sizes = {}
	sizes['all DNA'] = 0
	for(let[key, value] of Object.entries(data.metadata.fields)){

		if(!(accepted.includes(value.name))){
			delete data.metadata.fields[key]
		}
		else{
			sizes[value.name] = 0
			for (var l in value.lookup){
				sizes['all DNA'] += value.lookup[l].count
				sizes[value.lookup[l].value] =(value.lookup[l].count)
			} 
		}
	}
	data.children= y.children
	tree = []	
	if(data) {
		// create tree
		allBarcodes.map(b => {
		...
		piechartPlotSVG.selectAll('g.slice')
		piechartLegendSVG.selectAll('g.legendElement')
...
//	var selected = partition(tree).descendants().filter(function(d){ return d.data.taxid == 10239; })[0];
	...
	//--------------------------------------------
	// draw slices
	const part = partition(tree[selectedBarcode])
	...
	// .filter(function(d){
	// 		return accepted.includes(d.data.rank)
	// 	})
	...
	sliceEnter.append('title')
		.text(function(d) { 
			var ret = 'Label: ' + d.data.label + '\nReads: ' + formatNumber(d.data.value);
			if(d.data.segment) {
