...
while getopts "hi:s:o:l:w:x:m:" OPTION
do
	case $OPTION in
		h) usage; exit 1 ;;
...
if [[ ! -z $files_seen ]]; then
	# s="[";
	# for file in $files_seen; do    
	# 	s=$s$(printf "\t$file,\n"  )
	# done  
	# # files_seen_metadata="["$( echo $files_seen | sed -r 's/ +/,/g' )"]"
	# # echo $files_seen_metadata
	# s=$s"]"
	# echo $s
	arr=( $files_seen )
	for index in "${!arr[@]}"; 
	do
		arr[$index]=$(real ${arr[$index]}    )
	done  
	printf -v joined '\n\t\"%s\",' "${arr[@]}"
	# echo "[${joined%,}""]"
	files_seen_metadata="[${joined%,}""
]"
fi
...
gawk -F $'\t' \
	...
	-v FILES_SEEN="$files_seen_metadata" \
	-v HOSTNAME="$(hostname)" \
	-v LEVELS="$levels" \
